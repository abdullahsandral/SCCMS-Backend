...
const getTeacherSubjects= async (req, res, next) =>
{
    ...
    try 
    {
        teacherSubjects = await Subjects.findAll({where: {Teacher_ID: tID}})
   ...
   {   
       try 
        { await Attendance.create({Attendance_Status: oneRow.attendanceStatus, Attendance_Date: date,
            Class_ID: classID, Student_ID:oneRow.studentID, Subject_ID: subjectID})
  ...
  const {classID,subjectID,date,attendanceData} = req.body;
   ...
   {   
       try 
        ...
        {where: {Attendance_Date: date, Class_ID: classID, Student_ID:oneRow.studentID, Subject_ID: subjectID}} )
