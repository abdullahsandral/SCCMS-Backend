...
const getAllStudents = async (req, res, next) => 
{
    ...
    try 
    {
        allStudents  = await db.query('SELECT s.Student_ID, s.Roll_Number, s.First_Name, s.Last_Name, s.E_Mail,s.Contact_Number, c.Class_Name FROM `students` as s INNER JOIN classes as c on c.Class_ID=s.Class_ID ORDER BY `s`.`Student_ID` ASC',
...
const getStudentById = async (req, res, next) => 
{
    ...
    try 
    {
        student  = await Students.findOne({where : {Student_ID : studentId}});
...
const editStudentById = async (req, res, next) =>
{
    ...
    try 
    {
        ...
        if(password)  await Students.update({ Password: hashedPassword},{ where : { Student_ID : studentId}})
